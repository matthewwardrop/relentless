...
class GitAnnotate(XeLaTeX):
    ...
    def process(self, output='history', repo='.', annotate={}, branches=None):
        ...
        self.output.write( header% (len(branches),len(all_commits)) )

        for i, branch in enumerate(branches):
            self.output.write( r"\branch{%d}{%s}" % (i, prepare(branch)) + "\n" )
        ...
        for d,commit in enumerate(all_commits):
            ...
            self.output.write( r"\commit{%d}{%d}{%s}{%s}{%f}{%s}\\" % (i, d, commit.hexsha[:7], prepare(commit.message) + "\n", rel, value) )

        self.output.write( middle )

        for d,commit in enumerate(all_commits):
            for parent in commit.parents:
                self.output.write( r"\connect{%s}{%s};" % (parent.hexsha[:7], commit.hexsha[:7]) + "\n" )
